#!/usr/bin/env python
# -*- coding: utf-8 -*-
#Boa:Frame:hello_main
# JNMaster / hello / shrimp entry point -- just for demonstration
#
...
SHRIMP_INFO = {
    ...
    'desc':   'the hello world shrimp usable as gui starting point',
...
def shrimp_down(just_querying=False):
    if just_querying:
        ret = wx.MessageBox('leave this marine world?', 'aloha', wx.YES_NO)
        if ret == wx.YES:
    ...
    loginfo('hello teardown initiated')
    wx.CallAfter(_SELF_FRAME.Destroy)

def create(parent):
    global _SELF_FRAME
    _SELF_FRAME = hello_main(parent)
    return _SELF_FRAME

[wxID_HELLO_MAIN, wxID_HELLO_MAINBTNEXIT,
] = [wx.NewId() for _init_ctrls in range(2)]

class hello_main(wx.Frame):
    def _init_ctrls(self, prnt):
        # generated method, don't edit
        wx.Frame.__init__(self, id=wxID_HELLO_MAIN, name='', parent=prnt,
              pos=wx.Point(500, 107), size=wx.Size(400, 400),
              style=wx.DEFAULT_FRAME_STYLE, title=u'hello world')
        self.SetClientSize(wx.Size(384, 362))
        self.Bind(wx.EVT_CLOSE, self.OnHello_mainClose)

        self.btnExit = wx.Button(id=wxID_HELLO_MAINBTNEXIT,
        ...
        self.btnExit.Bind(wx.EVT_BUTTON, self.OnBtnExitButton,
              id=wxID_HELLO_MAINBTNEXIT)

    def __init__(self, parent):
        self._init_ctrls(parent)

    def OnHello_mainClose(self, event):
        loginfo('window close event, initiating subshrimp shutdown')
        ok_to_shutdown = cooker.query_shutdown('hello')
        if ok_to_shutdown:
            cooker.bring_down_shrimp('hello')
            event.Skip()
        else:
